 ...
 *
 * <p>Se ejecutan SOLO cuando la variable de entorno INTEGRATION=true. En CI se activan en el job de
 * integración (ver workflow).
 *
 * <p>Requieren: ChromeDriver o GeckoDriver instalado y en PATH. Usan HEADLESS=true y BROWSER según
 * las variables de entorno del runner.
 */
...
class ScraperE2ETest {
  ...
  @Test
  @Order(1)
  void e2eScraperExtraeAlMenos10Resultados() {
    List<ProductResult> resultados = MercadoLibreScraper.extraerDatos(driver, PRODUCTO_TEST);
  ...
  @Test
  @Order(2)
  void e2eTodosLosItemsTienenTituloYLink() {
    List<ProductResult> resultados = MercadoLibreScraper.extraerDatos(driver, PRODUCTO_TEST);
  ...
  @Test
  @Order(4)
  void e2eLinksSonUrlsAbsolutasHTTPS() {
    List<ProductResult> resultados = MercadoLibreScraper.extraerDatos(driver, PRODUCTO_TEST);
    ...
    resultados.stream()
        ...
        .forEach(
            p -> {
              URI uri =
                  assertDoesNotThrow(() -> new URI(p.getLink()), "link inválido: " + p.getLink());
              assertTrue(uri.isAbsolute(), "link no absoluto: " + p.getLink());
              assertTrue(uri.getScheme().startsWith("http"), "scheme inesperado: " + p.getLink());
            });
  }
}
